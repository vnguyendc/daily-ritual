// Meals Controller — CRUD + photo upload + AI analysis
...
import type { APIResponse, DailyNutritionSummary } from '../types/api.js'
...
export class MealsController {
  ...
  // POST /meals — multipart upload (photo + meal_type + date)
  static async createMeal(req: Request, res: Response) {
    try {
      ...
      // If a photo was uploaded, store it and analyze it
      if (file) {
        ...
        if (uploadError) {
          ...
          // Continue without photo — not a hard failure
        } else {
      ...
      // Fire-and-forget: generate post-meal insight
  ...
  // PUT /meals/:id — user corrections to calories/macros
  ...
  // GET /meals/daily-summary?date=YYYY-MM-DD

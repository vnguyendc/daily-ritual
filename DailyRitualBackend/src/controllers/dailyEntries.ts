...
export class DailyEntriesController {
  ...
  // Batch fetch daily entries for multiple dates (optimized for calendar views)
  static async getBatchDailyEntries(req: Request, res: Response) {
    try {
      const token = req.headers.authorization?.replace('Bearer ', '')
      const useMock = process.env.USE_MOCK === 'true'
      const devUserId = process.env.DEV_USER_ID
      ...
      // Get dates from query param (comma-separated) or body
      const datesParam = req.query.dates as string
      const dates: string[] = datesParam ? datesParam.split(',') : (req.body.dates || [])
      ...
      // Limit to prevent abuse (max 31 days at a time)
      if (dates.length > 31) {
  ...
  // Batch fetch daily entries with training plans (combined optimized query)
  static async getBatchDailyEntriesWithPlans(req: Request, res: Response) {
    try {
      const token = req.headers.authorization?.replace('Bearer ', '')
      const useMock = process.env.USE_MOCK === 'true'
      const devUserId = process.env.DEV_USER_ID

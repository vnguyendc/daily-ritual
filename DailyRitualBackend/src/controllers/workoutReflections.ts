...
export class WorkoutReflectionsController {
  ...
  // Create a new workout reflection
  static async createWorkoutReflection(req: Request, res: Response) {
    try {
  ...
  // Get workout reflections with pagination and filtering
  static async getWorkoutReflections(req: Request, res: Response) {
    try {
      const token = req.headers.authorization?.replace('Bearer ', '')
      if (!token) {
        return res.status(401).json({ error: 'Authorization token required' })
      ...
      const workoutType = req.query.workout_type as string
      const minFeeling = req.query.training_feeling_min ? parseInt(req.query.training_feeling_min as string) : undefined
  ...
  // Get a specific workout reflection
  static async getWorkoutReflection(req: Request, res: Response) {
    try {
      const token = req.headers.authorization?.replace('Bearer ', '')
      if (!token) {
        return res.status(401).json({ error: 'Authorization token required' })
  ...
  // Update a workout reflection
  static async updateWorkoutReflection(req: Request, res: Response) {
    try {
      const token = req.headers.authorization?.replace('Bearer ', '')
      if (!token) {
        return res.status(401).json({ error: 'Authorization token required' })
  ...
  // Delete a workout reflection
  static async deleteWorkoutReflection(req: Request, res: Response) {
    try {
      const token = req.headers.authorization?.replace('Bearer ', '')
      if (!token) {
        return res.status(401).json({ error: 'Authorization token required' })

...
export class DailyEntriesController {
    static async getDailyEntry(req, res) {
        try {
    ...
    static async completeEveningReflection(req, res) {
        try {
            const token = req.headers.authorization?.replace('Bearer ', '');
            const useMock = process.env.USE_MOCK === 'true';
            const devUserId = process.env.DEV_USER_ID;
            ...
            const eveningData = validationResult.data;
            const entry = await DatabaseService.createOrUpdateDailyEntry(user.id, date, {
...
//# sourceMappingURL=dailyEntries.js.map
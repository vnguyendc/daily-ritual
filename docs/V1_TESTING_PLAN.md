...
```json
{
  ...
  "planned_notes": "Notes/thoughts for the day"
}
```
...
- Endpoints:
  ...
  - iOS: Insights tab now fetches list and stats, supports mark‑as‑read.
...
- [ ] Morning Ritual (goals, gratitude, affirmation, notes) submits successfully; Evening Reflection submits successfully
...
- [ ] Insights tab loads list from `/insights` and stats from `/insights/stats`
- [ ] Mark as read updates server and UI badge count
...
- Implemented: Cached DailyEntry by date; queue and replay for Morning and Evening submits; queue for Training Plan create/update/delete; replay with exponential backoff on foreground; user-visible sync banner with pending count.
- Implemented: Local goal completion state per date (UI-only, does not write to server).
...
- [ ] Training plans offline: create/update/delete enqueue → after foreground resume, ops replay successfully
- [ ] 401 while offline → after sign-in, queued ops replay and succeed
- [ ] Sync banner shows "Syncing… X pending" during replay and updates count
- [ ] Tap "Sync Now" triggers replay immediately and reduces pending count
- [ ] Pull-to-refresh on Today and Training Plans triggers replay + refresh
